...
def SelfConv3D(image2darray_inputs,         # 3D Image2DArray: D x H x W -> Cgin*Din x Hin x Win x Win*4cin
    ...
    for d in range(Dout):
        for h in range(Hout):
            for w in range(Wout):
                out_loc = (d, h, w)
                SelfConv3D_helper(inputs, filters, outputs, 
...
def SelfConv3D_helper(inputs,               # image2dArray: Cgin*Din x Hin x Win*4cin
    ...
    # Calculate the Conv3D starting location (x = win, y = hin, z = din) in inputs
    inputX = outputX * strideX - padX   
    inputY = outputY * strideY - padY
    inputZ = outputZ * strideZ - padZ

    ...
    )
{
    ...
    for(int cog = 0; cog < Cgout; ++cog) {
                ...
                for(int kz = 0; kz < Dk; ++kz) {
                    ...
                    for(int ky = 0; ky < Hk; ++ky) {
                        ...
                        for(int kx = 0; kx < Wk; ++kx) {
                            ...
                            //if(ox == 0 && oy == 0 && oz == 0 && cog == 0 && idx == 0) {
                            //    printf("output=(%d,%d,%d,%d) input=(%d,%d,%d) inputValue=[%f,%f,%f,%f]\n", oz, oy, ox, cog, img2dArrCurrentIx, img2dArrCurrentIy, img2dArrCurrentIz, inputPatchValue.x, inputPatchValue.y, inputPatchValue.z, inputPatchValue.w);
                            //    printf("output=(%d,%d,%d,%d) filter=(%d,%d) filterValue=[%f,%f,%f,%f]\n", oz, oy, ox, cog, img2dFx, img2dCurrentFy, filterValue.x, filterValue.y, filterValue.z, filterValue.w);
                            //}
        ...
        conv3DResults += read_imagef(biases, smp, cog);
